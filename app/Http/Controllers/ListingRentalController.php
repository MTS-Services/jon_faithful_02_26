...
class ListingRentalController extends Controller
{
    ...
    /**
     * Store external rental listing link submission
     */
    public function storeLinkListing(Request $request): RedirectResponse
    {
        $validated = $request->validate([
    ...
    /**
     * Update the specified rental listing
     */
    public function updateListing(Request $request, $id): RedirectResponse
    {
        ...
        if ($request->hasFile('primary_image_url')) {
            ...
            if (
                ...
                Storage::disk('public')->delete('rentals/primary/' . $rental->primary_image_url);
            }

            $file = $request->file('primary_image_url');
            $imageName = time() . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
            $file->storeAs('rentals/primary', $imageName, 'public');
        ...
        if ($request->hasFile('gallery_images')) {
            ...
            // get old gallery
            $oldGallery = ListingGallery::where('listing_id', $rental->id)
            ...
            foreach ($oldGallery as $gallery) {
                if (
                    ...
                    Storage::disk('public')->delete('rentals/gallery/' . $gallery->image_url);
                }
            }
            ...
            foreach ($request->file('gallery_images') as $image) {
                $galleryUrl = time() . '_' . uniqid() . '.' . $image->getClientOriginalExtension();
                $image->storeAs('rentals/gallery', $galleryUrl, 'public');
    ...
    public function deleteListing($id): RedirectResponse
    {
        ...
        if ($rental->primary_image_url) {
            $oldPath = str_replace('/storage/', '', $rental->primary_image_url);
            Storage::disk('public')->delete($oldPath);
        }

        // Delete gallery images if you have a separate table
