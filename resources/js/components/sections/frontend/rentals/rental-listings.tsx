...
export default function RentalListings({ rentals, cities, filters = {} }: Props) {
    ...
    // Tracks whether a search/price change came from the user (not from prop sync)
    ...
    // Sync state when Inertia delivers new props — but do NOT mark as user input
    useEffect(() => {
    ...
    const buildFilterData = useCallback(() => {
        const filterData: any = {};
        if (searchTerm) filterData.search = searchTerm;
        if (selectedCities.length > 0) filterData.city = selectedCities.join(',');
        ...
        if (property_type.length > 0) filterData.property_type = property_type.join(',');
        if (priceMin) filterData.price_min = priceMin;
        if (priceMax) filterData.price_max = priceMax;
        return filterData;
    }, [searchTerm, selectedCities, bedrooms, bathrooms, square_feet, property_type, priceMin, priceMax]);

    const handleFilter = useCallback(() => {
        router.get(route('frontend.rentals'), buildFilterData(), {
            preserveState: true,
            preserveScroll: true,
        });
    }, [buildFilterData]);

    // Debounced effect — only fires when the user actually typed/changed price
    useEffect(() => {
        if (!isReady || !isUserTyping.current) return;
    ...
    }, [searchTerm, priceMin, priceMax, isReady, handleFilter]);
    ...
    const handleCityChange = (cityId: number) => {
        ...
        const filterData: any = {};
        if (searchTerm) filterData.search = searchTerm;
        if (newSelectedCities.length > 0) filterData.city = newSelectedCities.join(',');
        if (bedrooms) filterData.bedrooms = bedrooms;
        if (bathrooms) filterData.bathrooms = bathrooms;
        if (square_feet) filterData.square_feet = square_feet;
        if (property_type.length > 0) filterData.property_type = property_type.join(',');
    ...
    const handlePropertyTypeChange = (type: string) => {
        ...
        if (bathrooms) filterData.bathrooms = bathrooms;
        if (square_feet) filterData.square_feet = square_feet;
        if (newSelectedTypes.length > 0) filterData.property_type = newSelectedTypes.join(',');
        if (priceMin) filterData.price_min = priceMin;
        if (priceMax) filterData.price_max = priceMax;
    ...
    const handlePriceMinChange = useCallback((value: string) => {
        ...
        if (priceMax && Number(priceMax) < bounded) setPriceMax(String(bounded));
    }, [priceMax]);

    const handlePriceMaxChange = useCallback((value: string) => {
        isUserTyping.current = true;
        if (value === '') { setPriceMax(''); return; }
        ...
        if (priceMin && Number(priceMin) > bounded) setPriceMin(String(bounded));
    }, [priceMin]);
    ...
    // Intercept searchTerm changes from the input to mark as user-driven
    ...
    const getCityName = (cityId: number) =>
        cities?.find((city: any) => city.id === cityId)?.name || '';
                    ...
                    <div className="col-span-12 lg:col-span-4">
                        <div className="space-y-6">
                            <div className="rounded-xl bg-background p-4 shadow">
                                <h4 className="mb-3 font-montserrat font-semibold text-primary">Search</h4>
                                <input
                                    type="text"
                                    placeholder="Search..."
                                    value={searchTerm}
                            ...
                            <div className="mb-3 rounded-xl bg-background p-4 font-montserrat font-semibold text-primary shadow">
                                <h4 className="mb-3 font-semibold">City and/or Region</h4>
                                <div className="space-y-2">
                                    {cities?.map((city: any) => (
                                        <label key={city.id} className="flex cursor-pointer items-center gap-2 font-medium">
                                            <input
                                                type="checkbox"
                            ...
                            <div className="rounded-xl bg-background p-4 shadow">
                                <h4 className="mb-3 font-montserrat font-semibold text-primary">Bedrooms</h4>
                                <select
                            ...
                            <div className="rounded-xl bg-background p-4 shadow">
                                <h4 className="mb-3 font-montserrat font-semibold text-primary">Bathrooms</h4>
                                <select
                                    className="w-full rounded-md border border-primary px-4 py-3"
                                    value={bathrooms}
                            ...
                            <div className="rounded-xl bg-background p-4 shadow">
                                <h4 className="mb-3 font-montserrat font-semibold text-primary">Square Feet</h4>
                                <select
                                    className="w-full rounded-md border border-primary px-4 py-3"
                                    value={square_feet}
                            ...
                            <div className="rounded-xl bg-background p-4 shadow">
                                <h3 className="mb-3 font-montserrat font-semibold text-primary">Property Price</h3>
                                <div className="space-y-4">
                                    ...
                                    {(priceMin || priceMax) && (
                                        <button type="button" onClick={handlePriceClear} className="text-sm font-semibold text-primary underline">
                                            Clear price filter
                                        </button>
                                    )}
                                </div>
                            ...
                            <div className="mb-3 rounded-xl bg-background p-4 font-montserrat font-semibold text-primary shadow">
                                <h4 className="mb-3 font-semibold">Property Type</h4>
                                <div className="space-y-2">
                                    {propertyTypes.map((type) => (
                                        <label key={type.value} className="flex cursor-pointer items-center gap-2 font-medium">
                                            <input
                                                type="checkbox"
                    ...
                    <div className="col-span-12 mt-6 lg:col-span-8">
                        <div className="mb-4 flex flex-wrap gap-2">
                            {searchTerm && (
                                <span className="inline-flex items-center gap-2 rounded-full border border-primary px-4 py-2 font-montserrat font-semibold text-primary">
                                    Search: {searchTerm}
                                    <button onClick={() => setSearchTerm('')} className="hover:text-secondary">×</button>
                                </span>
                            )}
                            {selectedCities.map((cityId) => (
                                <span key={cityId} className="inline-flex items-center gap-2 rounded-full border border-primary px-4 py-2 font-montserrat font-semibold text-primary">
                                    City: {getCityName(cityId)}
                                    <button onClick={() => handleCityChange(cityId)} className="hover:text-secondary">×</button>
                                </span>
                            ))}
                            {property_type.map((type) => (
                                <span key={type} className="inline-flex items-center gap-2 rounded-full border border-primary px-4 py-2 font-montserrat font-semibold text-primary">
                                    {getPropertyTypeLabel(type)}
                                    <button onClick={() => handlePropertyTypeChange(type)} className="hover:text-secondary">×</button>
                                </span>
                            ))}
                            {(priceMin || priceMax) && (
                                <span className="inline-flex items-center gap-2 rounded-full border border-primary px-4 py-2 font-montserrat font-semibold text-primary">
                                    Price: {formatCurrency(Number(priceMin || PRICE_MIN_DEFAULT))} - {formatCurrency(Number(priceMax || PRICE_MAX_DEFAULT))}
                                    <button onClick={handlePriceClear} className="hover:text-secondary">×</button>
                                </span>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                            {rentals?.data?.map((rental: any) => (
                                <PlatinumCard key={rental.id} property={rental} type="rental" />
                            ))}
                        </div>
                                    ...
                                    return (
                                        <Link
                                            ...
                                            className={`rounded px-4 py-2 transition-colors ${
                                                link.active ? 'bg-secondary text-primary-foreground' : 'bg-primary text-primary-foreground hover:bg-secondary'
                                            } ${!link.url ? 'cursor-not-allowed opacity-50' : ''}`}
                                        >
                                            {isPrevious || isNext ? displayLabel : link.label.replace(/&laquo;|&raquo;/g, '')}
                                        </Link>
                                    );
                                })}
                            </div>
